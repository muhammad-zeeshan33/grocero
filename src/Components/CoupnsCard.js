...
const CoupnsCard = (props) => {
  ...
  return (
    <>
      <div
        class=' m-2 bg-[#FFF5F9] rounded-lg  shadow-sm  hover:bg-[#FACFE0] p-5 card '
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <div className=' relative '>
          <div className={ishovering ? "absolute -top-5 -right-5 " : "hidden"}>
            <button onClick={() => deleteCoupenHandler(props.id)}>
        ...
        <div class=' flex justify-end mt-2'>
          <input
            ref={inputRef}
            type='text'
            className='bg-white mr-2 outline-none border border-[#FACFE0] focus:ring-4 focus:ring-[#FACFE0]  text-sm rounded-lg w-full block p-2.5'
            value={props.promo}
          />
          <button
            onClick={copyToClipboard}
            class='copybtn text-white bg-[#D27095] focus:ring-4 focus:outline-none focus:ring-[#FACFE0] font-medium rounded-lg text-sm px-5 py-2.5 text-center'
          >
            {copySuccess}
          </button>
        </div>
